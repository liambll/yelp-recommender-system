...
commentbysub = reddit_json.map(lambda x: (x['subreddit'], (x['author'], float(x['score']))))

commentbysub_withAvg = commentbysub.join(subreddit_avg).map(lambda (subreddit,((author,score),avg)): (1.0*score/avg,author)).sortByKey(False)
